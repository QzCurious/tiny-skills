...
`Continuable` 是一個主題相對於 Intended Continuation 的一種 Property: 在預期的暫停或交接邊界後, 接續者仍能從屆時可取得的脈絡還原主題目前有效的接續狀態, 並在不發生實質扭曲的情況下繼續 Intended Work

Continuable 只反映目前有效的接續狀態; 不反映已不再影響接續的 origin, process history 或 discussion history
...
對 Intended Continuation 而言, 邊界後仍可取得的脈絡應足以使接續者還原:
...
接續者應能據此在不改變主題實質意義與有效接續方向的情況下繼續 Intended Work
...
歷史內容只有在仍具有 current governing force, 或必須保留 supersession fact 才能避免錯誤接續時, 才屬於目前有效的接續狀態
